...
ShapeData ShapeGenerator::makePlaneUnseamedIndices(uint tesselation)
{
	...
	ret.numIndices = dimensions * 2 * 3; //2 triangle per square, 3 indices per triangle
	...
	for (int row = 0; row < tesselation; row++)
	{
		...
		//This code is crap but works, and Im not in the mood right now to clean it up
		for (int col = 0; col < tesselation; col++)
		{
			//bottom left triangle
			ret.indices[runner++] = tesselation * row + col;
			//One row down unless its the bottom row
			ret.indices[runner++] = (row + 1 == tesselation ? 0 : tesselation * row + tesselation) + col;
			//Move to vert right of this one unless its the last vert
			...
			ret.indices[runner++] = (tesselation * row + col + tesselation + (col + 1 == tesselation ?
				-tesselation + 1 : 1)) % dimensions;
			...
			else
			{
				...
				//the % dimensions at the end accounts for the last row hookings to the first row
...
ShapeData ShapeGenerator::makeArrow()
{
	...
	Vertex stackVerts[] =
	{
		...
		//Buttom side of back of arrow tip
...
{
	float * B = new float[4 * (grid + 1)]; //Pre Computed Berstien basis Function
	float * dB = new float[4 * (grid + 1)]; //Pre Computed dirivtives of basis fuin

	int idx = 0, elIndex = 0, tcIndex = 0;

	//Pre-compute the basis funcitons (Bernstien polynomails)
	//and there derivates
...
//Check
glm::vec3 ShapeGenerator::evaluateNormal(int gridU, int gridV,
...
ShapeData ShapeGenerator::makeTorus(uint tesselation)
{
...

	{
		...
		glm::mat4 transform =
			glm::rotate(glm::mat4(), glm::radians(round1 * sliceAngle), glm::vec3(0.0f, 1.0f, 0.0f)) *
...
ShapeData ShapeGenerator::makeSphere(uint tesselation)
{
	...
	for (size_t col = 0; col < dimensions; col++)
	{
		...
		for (size_t row = 0; row < dimensions; row++)
		{
			...
			size_t vertexIndex = col * dimensions + row;
			Vertex& v = ret.vertices[vertexIndex];
			v.position.x = RADUIS * cos(phi) * sin(theta);
			v.position.y = RADUIS * sin(phi) * sin(theta);
			v.position.z = RADUIS * cos(theta);
